...
- 🤖 可选 AI 文章摘要，自动生成独立的 description，写博客就专心写正文~
...
正文写完了，description 也可以交给 AI~ 在 `site.config.js` 的 `ai_desc_gen` 中调整提示词和生成规则，每个选项都有注释。默认 `auto`：不配 Key 就用正文摘录，照常部署，不影响写博客。
